...
import { useLanguage } from '@/components/language-provider'; // <--- Correct
...
export function Sidebar({ className, userSession }: { className?: string, userSession?: any }) {
  const { t } = useLanguage(); // 1. Get translations
  const [isOpen, setIsOpen] = useState(false);
  const pathname = usePathname();

  // 2. Define items INSIDE so they can use 't'
  const navItems = [
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-[#004D98] text-white border-r border-[#A50044]">
      ...
      <div className="p-4 border-t border-[#A50044] bg-[#003870] space-y-4">
        ...
        {/* 3. ADDED THE LANGUAGE SWITCHER HERE */}
        ...
        {userSession ? (
             <Button 
                ...
                variant="ghost" 
                className="w-full justify-start text-white hover:bg-[#A50044] hover:text-[#EDBB00] uppercase
